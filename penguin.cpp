...
const float SPINNER_SPEED = 0.01;
...
void drawCube();
void drawFrustum(float top, float bot, float height);
void drawPrism(float len, float h);
void drawTrapezoid(float t, float b, float l, float h);

void pgRender(void (*fun)());
void pgDrawBody();
void pgDrawHead();
void pgDrawBeak();
void pgDrawLeg();
...
void initGlui()
{
	...
	glui_light = GLUI_Master.create_glui("Light Control", 0, 600, Win[1] + 64);

	glui_panel = glui_light->add_panel("Light");

	glui_spinner = glui_light->add_spinner_to_panel(glui_panel, "Light Degree Theta (* PI):", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::LIGHT_THETA));
...
void animate()
{
	...
	{
		Vector newDOFs = getInterpolatedJointDOFS(animationTimer->elapsed());
		joint_ui_data->setDOFVector(newDOFs);
...
//Render Polygon by different render style. *fun is the pointer of the drawing function.
void pgRender(void (*fun)()){

	GLfloat dif1[] = { 0.01, 0, 0, 1 };
	GLfloat amb1[] = { 0.5, 0, 0, 1 };
	GLfloat spe1[] = { 0.5, 0, 0, 1 };

	GLfloat dif2[] = { 0.5, 0, 0, 1 };
	GLfloat amb2[] = { 0.5, 0, 0, 1 };
	GLfloat spe2[] = { 0.01, 0, 0, 1 };
...
void pgDrawSoulder(){
	drawTrapezoid(0.75,0.5,2,0.1);
}

void pgDrawHand(){
	drawTrapezoid(0.5, 0.5, 0.5, 0.1);
}
...
// Draw a Frustum, centered at the current location
// With top length, bot length, and height
void drawFrustum(float top, float bot, float height)
{
	float k1 = (bot - top) / (2 * height);
	float k = k1 / sqrt(k1 * k1 + 1);
	float u = 1 / sqrt(k1 * k1 + 1);

	glBegin(GL_QUADS);
		// draw front face
		glNormal3f(0, k, u);
		glVertex3f(-bot, -height, bot);
		glVertex3f( bot, -height, bot);
		glVertex3f( top,  height, top);
		glVertex3f(-top,  height, top);
...
// Draw a Prism with length of the trangle and height
void drawPrism(float len, float h)
{
	float a = len / 2;
	float b = a * 1.732;
...
// Draw a Trapezoid with top length, bot length, vertical length and height
void drawTrapezoid(float t, float b, float l, float h){
